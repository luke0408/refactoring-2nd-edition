...
[ 리팩터링과 같이 사용하기 좋은 프로세스 ]

- 익스트림 프로그래밍(XP)
- 테스트 주도 개발(TDD)

책에서는 위 두개의 프로세스를 소개하고 있습니다. 특히, XP는 최초의 애자일 소프트웨어 방법론 중 하나로 등장하여 애자일의 부흥을 이끌었죠.

지금은 상당수의 프로젝트가 애자일을 적용하고 있으나 동시에 애자일을 제대로 사용하지 못하고 있다합니다. 애자일을 제대로 적용하려면 리팩터링에 대한 팀의 역량과 열정이 뒷받침되어 프로세스 전반에 리팩터링이 자연스럽게 스며들어야 하는데 그렇지 못하기 때문이죠.

리팩터링의 첫 토대는 자가 테스트 코드입니다. 

또한 팀으로 개발하면서 리팩터링을 적극적으로 사용할 수 있으려면 다른 사람의 작업을 방해하지 않을 수 있어야 합니다. 이는 지속적 통합(CI)가 중요한 이유입니다.

따라서 자가 테스트 코드, 지속적 통합, 리팩터링이라는 세 기법은 서로 강력한 상승효과를 가져다줍니다.

## 2.8 리팩터링과 성능

'직관적인 설계 vs 성능'은 중요한 문제죠.

실제로 소프트웨어를 이해하기 쉽게 하는 방향으로 리팩터링을 하다보면 성능이 안좋아지는 경우들이 있습니다. 하지만 그와 동시에 성능을 튜닝하기 더 쉽게 됩니다.

대부분의 프로그램은 전체 코드 중 극히 일부에서 대부분의 시간을 소비한다는 흥미로운 사실이 있습니다. 즉, 전체 코드를 고르게 최적화한다면 그중 90% 정도는 효과가 거의 없다는 것이죠.

이런 통계치에 따라 성능을 향상시키기 위해 모든 코드를 최적화하기 보다는 다음과 같은 방법을 생각해볼 수 있습니다.

1. 성능 최적화에 돌입하기 전까지는 성능보다는 다루기 쉬운 코드를 만드는데 집중한다. 
2. 성능 최적화에 돌입하면 먼저 프로파일러로 프로그램을 분석하여 시간과 공간을 많이 잡아먹는 지점을 찾는다.
3. 성능에 큰 영향을 주는 작은 부분들을 최적화 한다.

이렇게 하면 성능에 큰 영향을 주는 부분만 집중적으로 최적화 할 수 있게됩니다.

즉, 단기적으로 보면 리팩터링 단계에서 성능이 느려질 수도 있지만, 결과적으로 리팩터링에 의해 최적화가 쉬워지기 때문에 결과적으로 더 빠른 소프트웨어를 얻을 수 있게됩니다.
