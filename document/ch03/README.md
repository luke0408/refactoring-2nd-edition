...
주석은 코드에 악취가 아닌 향기를 입힙니다. 다만, 주석을 탈취제 처럼 사용하는 것은 문제가 될 수 있습니다. 주석이 장황하게 달린 코드는 그 원인이 코드를 잘못 작성했기 때문인 경우가 의외로 많습니다.

주석을 사용하지 말라는 이야기는 아니지만. `주석을 남겨야겠다는 생각이 들면, 가장 먼저 주석이 필요 없는 코드로 리팩터링 해보는 것`은 어떨까요?

- 특정 코드 블록이 하는 일에 주석을 남기고 싶다면, [함수 추출하기]()를 적용합니다.
- 이미 추출되어있는 함수임에도 여전히 설명이 필요하다면, [함수 선언 바꾸기]()로 이름을 바꿔봅니다.
- 시스템이 동작하기 위한 선행 조건을 명시하고 싶다면, [어서션 추가하기]()를 사용해도 좋습니다.
