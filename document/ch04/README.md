...
이 장에서 사용한 테스트는 단위 테스트(Unit Test)로, 코드의 작은 영역만 집중적으로 검증하지만 컴포넌트 간 상호작용까지는 다루지 않는 한정된 테스트 방식을 의미합니다. 

모든 자가 검증 테스트는 단위 테스트에서 출발하며, 이외에도 통합 테스트나 성능 테스트 등 다양한 종류의 테스트가 존재합니다.

테스트를 진행할 때 항상 명심해야 할 점은 처음부터 완벽한 테스트 케이스를 갖추기는 어렵다는 사실입니다. 그러므로 버그 리포트를 받으면 해당 상황을 재현할 수 있는 테스트 케이스부터 작성하는 것이 중요합니다. 

많은 사람들이 테스트를 어느 정도까지 진행해야 하는지 궁금해하지만, 테스트 커버리지는 코드에서 테스트하지 않은 부분을 찾아내는 도구일 뿐, 코드 전체를 완벽하게 검증했다고 보장하지 않습니다. 

따라서 높은 커버리지가 모든 경우를 테스트했다는 의미는 아니며, 지속적으로 테스트를 개선하고 추가하는 노력이 필요합니다.